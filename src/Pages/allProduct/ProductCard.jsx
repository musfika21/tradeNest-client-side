...
const ProductCard = ({ product }) => {
    ...
    return (
        <Link to={`/product-Details/${_id}`}>
            <div className={`group relative rounded-2xl overflow-hidden ${theme
                ...
                <div className="relative overflow-hidden h-48 sm:h-52">
                    ...
                    {category && (
                        <div className="absolute top-3 left-3 z-20">
                            <span className={`px-3 py-1.5 text-xs font-bold rounded-full backdrop-blur-md ${theme
                    ...
                    {/* Wishlist Button */}
                    <button className={`absolute top-3 right-3 z-20 p-2.5 rounded-full backdrop-blur-md ${theme ? 'bg-white/90 hover:bg-red-500' : 'bg-gray-800/90 hover:bg-red-500'
                    ...
                    <div className={`absolute bottom-0 right-0 w-20 h-20 ${theme ? 'bg-[#3E3F29]' : 'bg-[#BCA88D]'
                ...
                <div className="p-4 space-y-3 relative">
                    ...
                    {brand && (
                        <div className="flex items-center gap-2">
                            ...
                            <span className={`text-xs font-semibold uppercase tracking-wider ${theme ? 'text-gray-500' : 'text-gray-400'
                                }`}>
                                {brand}
                            </span>
                        </div>
                    ...
                    {/* Product Name */}
                    <h3 className={`font-bold text-sm line-clamp-2 min-h-[40px] leading-snug ${theme ? 'text-[#3E3F29]' : 'text-white'
                        } group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r ${theme ? 'group-hover:from-[#3E3F29] group-hover:to-[#5a6040]' : 'group-hover:from-[#BCA88D] group-hover:to-[#d4c4a8]'
                    ...
                    {/* Description */}
                    {description && (
                        <p className={`text-xs line-clamp-2 ${theme ? 'text-gray-600' : 'text-gray-400'
                            }`}>
                            {description}
                        </p>
                    )}
                    ...
                    <div className={`h-px bg-gradient-to-r ${theme
                    ...
                    <div className="flex items-center justify-between pt-2">
                            ...
                            <div className="flex items-center gap-1.5 group/rating">
                                <div className="flex items-center">
                                    {[...Array(5)].map((_, i) => (
                                        <FaStar
                                            key={i}
                                            className={`text-xs transition-all duration-200 ${i < Math.floor(rating)
                                ...
                                <span className={`text-xs font-bold ${theme ? 'text-gray-700' : 'text-gray-300'
                                    }`}>
                                    {rating}
                                </span>
                            </div>
                        ...
                        {price && (
                            <div className="flex items-baseline gap-1">
                                <span className={`text-xs font-medium ${theme ? 'text-gray-500' : 'text-gray-400'
                                    }`}>
                                    $
                                </span>
                                <span className={`text-xl font-black tracking-tight ${theme ? 'text-[#3E3F29]' : 'text-[#BCA88D]'
                ...
                <div className="absolute top-0 left-0 w-16 h-16 overflow-hidden pointer-events-none">
                    <div className={`absolute -top-8 -left-8 w-16 h-16 rounded-full ${theme ? 'bg-[#3E3F29]' : 'bg-[#BCA88D]'
